''' usage: python merge_dataset_plan_to_replay_buffer.py -i data/dataset/fold_pink_towel_lyl_right_v3 -e data/dataset/fold_pink_towel_hjc_best -o data/dataset/fold_pink_towel_merged_dataset_v1'''
...
@click.option('-e', '--exist_zarr', type=str, required=True, help='add more episode into this zarr')
@click.option('-o', '--output', required=True, help='folder path')
...
def main(input, exist_zarr, output, out_res, out_fov, compression_level, 
         no_mirror, mirror_swap, num_workers):
    output = os.path.join(output, 'dataset.zarr.zip')
    ...
    dataset_path = os.path.join(ROOT_DIR, exist_zarr, 'dataset.zarr.zip')
    ...
    n_grippers = None
    n_cameras = None
    buffer_start = out_replay_buffer['robot0_eef_pos'].shape[0]
    all_videos = set()
    vid_args = list()
    for ipath in input:
    ...
    for cam_id in range(n_cameras):
        ...
        dataset = out_replay_buffer.data[name]
        new_shape = (out_replay_buffer['robot0_eef_pos'].shape[0],) + out_res + (3,)
