...
class TrainDiffusionUnetImageWorkspace(BaseWorkspace):
    ...
    def val(self):
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            saved_folder = os.path.join(self.output_dir, "vis_on_trained_image", f"{batch_idx}")
            if not os.path.exists(saved_folder):
                os.makedirs(saved_folder)
            image0 = batch['obs']['camera0_rgb'][0, 0]
            ...
            image1 = batch['obs']['camera0_rgb'][0, 1]
            ...
            with open(file_path, 'w') as file:
                ...
                file.write(f'action_mse_error, {torch.nn.functional.mse_loss(pred_action, gt_action)}\n') 
                file.write(f'action_mse_error_pos, {torch.nn.functional.mse_loss(pred_action[..., :3], gt_action[..., :3])}\n')
                file.write(f'action_mse_error_rot, {torch.nn.functional.mse_loss(pred_action[..., 3:9], gt_action[..., 3:9])}\n')
                file.write(f'action_mse_error_width, {torch.nn.functional.mse_loss(pred_action[..., 9], gt_action[..., 9])}\n') 
